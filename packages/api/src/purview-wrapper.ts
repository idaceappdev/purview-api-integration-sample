...
/**
 * Retrieves label-details (incl. rights) for a given user.
 * The Microsoft Graph base-URL is taken from the environment variable
 * `GRAPH_BASE_URL`; if not set, it falls back to
...
export async function getLabelInfo(
  ...
  try {
    ...
    const filter = `id eq '${labelId}' and rights/ownerEmail eq '${userName}'`;
    const url = `${graphBaseUrl}/security/dataSecurityAndGovernance/sensitivityLabels?$expand=rights&$filter=${filter}`;
